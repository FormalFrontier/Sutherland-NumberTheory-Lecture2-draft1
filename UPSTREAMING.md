...
| 02_20a_Discussion | `exists_noninvertible_fractionalIdeal` | `02_20a_Discussion.lean:49` | **Reject** | Phase 2 verdict: The theorem is existential and non-constructive — it doesn't explicitly construct the non-invertible ideal, just extracts one via contrapositive of `isDedekindDomain_iff_isDedekindDomainInv`. The real content is `¬IsDedekindDomain (Polynomial ℤ)` (same as 02_12). Once that exists, this is a trivial 3-line corollary. Mathlib has the iff (`isDedekindDomain_iff_isDedekindDomainInv`) and `mul_inv_cancel_iff_isUnit`; no counterexamples exist but this existential wrapper adds no independent value. |
